...
```{r}
Model_1 <- lm(log(SalePrice) ~ YearBuilt + log(GrLivArea)  + Neighborhood + GarageCars + OverallQual + OverallCond + OverallQual*OverallCond +TotRmsAbvGrd + TotRmsAbvGrd*GarageCars  ,  data = new_Iowa_train)
...
```{r}
##Kaggle placement for this model: 2579th place, score of 0.15087
```
...
```{r}
new_Iowa_train$GarageYrBlt <- as.integer(new_Iowa_train$GarageYrBlt)

new_Iowa_train$MasVnrArea <- as.integer(new_Iowa_train$MasVnrArea)

new_Iowa_train$BsmtFullBath <- as.integer(new_Iowa_train$BsmtFullBath)

new_Iowa_train$BsmtHalfBath <- as.integer(new_Iowa_train$BsmtHalfBath)

new_Iowa_train$BsmtFinSF1 <- as.integer(new_Iowa_train$BsmtFinSF1)

new_Iowa_train$BsmtFinSF2 <- as.integer(new_Iowa_train$BsmtFinSF2)

new_Iowa_train$TotalBsmtSF <- as.integer(new_Iowa_train$TotalBsmtSF)
```

```{r}
new_Iowa_test$GarageYrBlt <- as.integer(new_Iowa_test$GarageYrBlt)

new_Iowa_test$MasVnrArea <- as.integer(new_Iowa_test$MasVnrArea)

new_Iowa_test$BsmtFullBath <- as.integer(new_Iowa_test$BsmtFullBath)

new_Iowa_test$BsmtHalfBath <- as.integer(new_Iowa_test$BsmtHalfBath)

new_Iowa_test$BsmtFinSF1 <- as.integer(new_Iowa_test$BsmtFinSF1)

new_Iowa_test$BsmtFinSF2 <- as.integer(new_Iowa_test$BsmtFinSF2)

new_Iowa_test$TotalBsmtSF <- as.integer(new_Iowa_test$TotalBsmtSF)
```
...
```{r}
##Kaggle placement for this model: 2545th place, score of 0.14973 score of 0.14973
```
