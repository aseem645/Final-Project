...
##XGBoost
```{r}
numerical_matrix <- sparse.model.matrix(SalePrice ~ .-1, data = new_Iowa_train)
```


```{r}
require(xgboost)
data(new_Iowa_train, package = "xgboost")
data(new_Iowa_test, package = "xgboost")
dtrain <- xgb.DMatrix(new_Iowa_train, label = new_Iowa_train$SalePrice)
dtest <- xgb.DMatrix(new_Iowa_test, label = new_Iowa_test$SalePrice)
